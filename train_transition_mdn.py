...
def initialize_model(device, **kwargs):
    ...
    model = MixtureDensityNetwork(kwargs["embedding_size"],kwargs["embedding_size"],256,512)
...
def train_transition_model(**kwargs):
    ...
    regression_model = RegressionWrapper(model, kwargs['embedding_size'])
    ...
    for epoch in tqdm(range(start_epoch, epochs), leave=True):
        ...
        for batch_no, batch in enumerate(tqdm(train_dataset, leave=True, mininterval=10.0)):
            ...
            if nans.any():
                print(f"NaN loss detected at epoch {fractional_epoch}, batch {batch_no}, {nans.sum()} nans detected.")
            loss = loss.nanmean()
        ...
        if val_loss < min_val_loss:
            ...
            best_val = {k:v.to('cpu') for k, v in model.state_dict().items()}
        if train_loss < min_train_loss:
            min_train_loss_epoch = epoch
            min_train_loss = train_loss
            best_train = {k:v.to('cpu') for k, v in model.state_dict().items()}
        if epoch % 10 == 9:
            regression_model.model.load_state_dict(OrderedDict(best_val))
            checkpoint = {
            ...
            regression_model.model.load_state_dict(OrderedDict(best_train))
            checkpoint = {
        ...
        elif val_loss > initial_val_loss:
            print(f"Validation loss exploded on epoch {epoch}, terminating training.")
            break
...
if __name__ == "__main__":
    ...
    parser.add_argument("--wandb_proj", type=str, help="Wandb project name", default="lm-sys_transition_mdn")
    ...
    parser.add_argument("--out_dir", type=str, help="Output directory for models", default="models/mdn")
    ...
    if args["use_wandb"]:
        wandb.setup()
    for i in range(2):#,len(start_steps)):
        start_step, transition_type = start_steps[i], types[i]
