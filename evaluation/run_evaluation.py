...
parser.add_argument("--cuda_for_all",  help="cuda", default="0")
...
cuda_q_embedding = convert(args["cuda_for_all"])
cuda_transition = convert(args["cuda_for_all"])
cuda_llm = convert(args["cuda_for_all"])
cuda_reward = convert(args["cuda_for_all"])
...
for agent,type in zip(agents, agent_type):
    ...
    print("time taken for 5 trials: ", time_taken)
    print("convo generated in total: ", convo_generated)
